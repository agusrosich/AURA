...
:: Modified to use local TotalSegmentatorV2 from zip file
:: ========================================================================
...
:: Check if local TotalSegmentatorV2 zip file exists
if not exist "models\TotalSegmentatorV2-master.zip" (
    echo %RED%Error: TotalSegmentatorV2-master.zip not found!%RESET%
    echo Please make sure the TotalSegmentatorV2-master.zip file is present in the models directory
    echo Expected location: %~dp0models\TotalSegmentatorV2-master.zip
    pause
    exit /b 1
)

echo %GREEN%Found local TotalSegmentatorV2-master.zip in models directory%RESET%
...
set "TEMP_EXTRACT_DIR=%~dp0models\totalsegmentator_temp"
...
echo 7. Local TotalSegmentatorV2 (from zip file)
echo.
...
echo.
echo %CYAN%Step 7b/7: Installing local TotalSegmentatorV2 from zip...%RESET%
...
powershell -command "Expand-Archive -Path 'models\TotalSegmentatorV2-master.zip' -DestinationPath '%TEMP_EXTRACT_DIR%'"
if !errorlevel! neq 0 (
    echo %RED%Error: Failed to extract TotalSegmentatorV2-master.zip!%RESET%
    pause
    exit /b 1
)

:: Find the extracted folder (in case it has a different name)
for /d %%i in ("%TEMP_EXTRACT_DIR%\*") do set "EXTRACTED_FOLDER=%%i"
...
:: Copy the extracted TotalSegmentatorV2 to site-packages
echo %YELLOW%Copying TotalSegmentatorV2 files to virtual environment...%RESET%
xcopy /E /I /Y "!EXTRACTED_FOLDER!\*" "%VENV_SITE_PACKAGES%\totalsegmentator\" >nul
if !errorlevel! neq 0 (
    echo %RED%Error: Failed to copy TotalSegmentatorV2 files!%RESET%
    echo %YELLOW%Please check that the extracted files in %TEMP_EXTRACT_DIR% are valid%RESET%
    pause
    exit /b 1
) else (
    echo %GREEN%TotalSegmentatorV2 files copied successfully!%RESET%
)
...
if exist "!EXTRACTED_FOLDER!\requirements.txt" (
    ...
    if !errorlevel! neq 0 (
        ...
        echo %GREEN%TotalSegmentatorV2 requirements installed successfully%RESET%
    )
)

:: Install setup.py if it exists
if exist "!EXTRACTED_FOLDER!\setup.py" (
    ...
    if !errorlevel! neq 0 (
        ...
        echo %GREEN%TotalSegmentatorV2 installed via setup.py successfully%RESET%
    )
) else (
    echo %YELLOW%No setup.py found, using direct copy installation%RESET%
)
...
echo ^)
echo.
echo :: Add TotalSegmentator to Python path
...
echo     if exist "%VENV_SITE_PACKAGES%\totalsegmentator" rmdir /s /q "%VENV_SITE_PACKAGES%\totalsegmentator"
echo     mkdir "%TEMP_EXTRACT_DIR%"
echo     powershell -command "Expand-Archive -Path 'models\TotalSegmentatorV2-master.zip' -DestinationPath '%TEMP_EXTRACT_DIR%'"
echo     for /d %%%%i in ("%TEMP_EXTRACT_DIR%\*") do set "EXTRACTED_FOLDER=%%%%i"
echo     mkdir "%VENV_SITE_PACKAGES%\totalsegmentator"
echo     xcopy /E /I /Y "%%EXTRACTED_FOLDER%%\*" "%VENV_SITE_PACKAGES%\totalsegmentator\"
echo     if exist "%%EXTRACTED_FOLDER%%\requirements.txt" pip install -r "%%EXTRACTED_FOLDER%%\requirements.txt" --no-warn-script-location
echo     if exist "%%EXTRACTED_FOLDER%%\setup.py" ^(
echo         pushd "%%EXTRACTED_FOLDER%%"
echo         python setup.py install
echo         popd
echo     ^)
echo     rmdir /s /q "%TEMP_EXTRACT_DIR%"
echo     echo TotalSegmentator files updated from local zip.
echo ^) else ^(
...
echo if exist "models\TotalSegmentatorV2-master.zip" ^(
echo     echo Local TotalSegmentatorV2 zip file exists
echo ^) else ^(
echo     echo Local TotalSegmentatorV2 zip file NOT found
echo ^)
echo.
echo deactivate
echo pause
...
echo %YELLOW%TotalSegmentatorV2 is installed from the local zip file%RESET%
echo.
pause